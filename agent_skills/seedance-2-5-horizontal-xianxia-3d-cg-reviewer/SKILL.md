...
2. **timing_math**：每组显式整数 4–30 秒，每镜为整数秒，镜头合计等于组总时长，镜头数一致；不得套旧时长或小数时间轴。无对白的高冲击纯动作若超过约 12 秒，必须复核是否确有持续流程、多个稳定状态或结果确认支撑，不能只靠悬停、慢动作或无剧情余波填满。
...
5. **xianxia_vfx_provenance**：核心效果（改变剧情/对象状态/空间边界或跨组持续）逐项核对连续时间轴正文中的来源、形态、路径、作用对象、反馈、收束、声音对应；局部余光、火星和环境底色只核对其实际可见因果，不因缺少无关字段制造 hard issue；具体能力均由剧本/项目包授权，不要求独立的视觉峰值字段。
...
- 因引用静态资产而省略核心效果的形态、路径、中间状态、作用边界、反馈、收束或同步声纹，导致动态过程不可追踪；尤其检查灵火迁移、禁制表现、材料状态变化、局部回溯边界和纹样传播。对不改变状态的微小余光、火星或环境底色，按其实际可见因果做最小核对。
...
- 合法短组虽然独立，但与相邻同空间同目标轻节点合并可能更稳定；不能因此反向扩成长组。
- 无对白的抢夺、命中、击碎、追逃或遁入组超过约 12 秒，但没有多个稳定状态或结果确认；应回看自然动作容量，避免成片呈慢放节奏。
- 一个生成单元同时跨越两个以上空间/意识层，并叠加核心 VFX、身份状态切换或多人重排；应在稳定状态处拆组，不能用长时长掩盖密度问题。
- 资产的动作尺度或状态与正文冲突，例如把工作态大件直接写成手持，或用换装/新脸代替肉身到魂体；应补状态过渡或改绑定，不让模型自行猜测。
