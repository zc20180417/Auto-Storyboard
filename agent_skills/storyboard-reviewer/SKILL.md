...
台词语速合格不代表片段节奏合格。强节拍数量只用于判断容量，不是自动判错依据。若同一组强节拍彼此并列、抢画面焦点，且没有足够表演时间，应判 `generation_density`；若它们属于同一空间、同一冲突目标、同一因果压力下的连续事件链，且每个时间段只承载一个主动作 / 一个连续对话节拍，不应仅因强节拍数量多而判错。
...
generator 允许在剧情峰值节拍上使用有动机的镜头语言（见 generator §二.9）。审核时必须把“有动机的 craft”和“凑时长 / 运镜污染”区分开，不要把合法手段判成 hard issue：

- **有动机镜头运动**：单组内 ≤1 个、写成整镜连续运动（如“中景缓推近景”）、用于峰值节拍、且属缓推/缓拉/轻微手持/轻摇等稳定运动的，属合法表达，不判 `generation_density` 过载，也不判 `prompt_pollution`。仍判 hard 的情况：同组多个运镜；写成段内硬切（“切至/画面转至”）；复杂或快速甩镜等不稳定运动；或运镜被用来掩盖动作过载。
...
- `generation_density`：强节拍过载、复合动作链时间不足、低密度撑时长、相邻流程节点拆碎、用普通状态动作硬凑 10 秒。
