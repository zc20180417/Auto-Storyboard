...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\jiaqianyi-dalao-baohong-scene\episodes\ep36"
```
...
```json
{
  "episode_id": "ep36",
  "status": "done",
  "output_name": "嫁给千亿大佬后我发疯爆红-ep36-agent-cli-storyboard.txt",
...
- Work only inside `episodes\ep36`. Treat project-level skill files and `../../context.md` as read-only.
