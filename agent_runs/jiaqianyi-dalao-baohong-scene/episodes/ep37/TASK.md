...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\jiaqianyi-dalao-baohong-scene\episodes\ep37"
```
...
```json
{
  "episode_id": "ep37",
  "status": "done",
  "output_name": "嫁给千亿大佬后我发疯爆红-ep37-agent-cli-storyboard.txt",
...
- Work only inside `episodes\ep37`. Treat project-level skill files and `../../context.md` as read-only.
