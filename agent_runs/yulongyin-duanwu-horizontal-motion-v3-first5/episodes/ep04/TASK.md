...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-v3-first5\episodes\ep04" --pre-check
```

For segment-level pre-check (scene mode, validate a segment draft before review):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-v3-first5\episodes\ep04" --pre-check --content-file "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-v3-first5\episodes\ep04/segments/segXX/draft.txt"
```

Full validation command (run after review.txt and status.json are written):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-v3-first5\episodes\ep04"
```
...
- `output_name`: `雨龙吟端午-ep04-agent-cli-storyboard.txt`
...
- Horizontal outputs must include `运镜设计：` under every time-coded segment; fixed camera is allowed only when the line explains how it stabilizes dialogue, blocking, props, or horizontal space. Horizontal `组首空间锁定` must be a static first-frame state, not an action route; put entering, walking, crawling, charging, approaching, taking, or placing actions into later `镜头描述` lines.
- Work only inside `episodes\ep04`. Treat project-level skill files and `../../context.md` as read-only.
