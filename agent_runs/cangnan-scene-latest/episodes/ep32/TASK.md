...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\cangnan-scene-latest\episodes\ep32" --pre-check
```

For segment-level pre-check (scene mode, validate a segment draft before review):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\cangnan-scene-latest\episodes\ep32" --pre-check --content-file "G:\Auto-Storyboard\agent_runs\cangnan-scene-latest\episodes\ep32/segments/segXX/draft.txt"
```

Full validation command (run after review.txt and status.json are written):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\cangnan-scene-latest\episodes\ep32"
```
...
- `output_name`: `苍南风云-ep32-agent-cli-storyboard.txt`
...
- Work only inside `episodes\ep32`. Treat project-level skill files and `../../context.md` as read-only.
