...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-first5\episodes\ep04" --pre-check
```

For segment-level pre-check (scene mode, validate a segment draft before review):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-first5\episodes\ep04" --pre-check --content-file "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-first5\episodes\ep04/segments/segXX/draft.txt"
```

Full validation command (run after review.txt and status.json are written):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\yulongyin-duanwu-horizontal-motion-first5\episodes\ep04"
```
...
- Horizontal outputs must include `运镜设计：` under every time-coded segment; fixed camera is allowed only when the line explains how it stabilizes dialogue, blocking, props, or horizontal space.
