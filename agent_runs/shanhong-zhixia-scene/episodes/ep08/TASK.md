...
```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\shanhong-zhixia-scene\episodes\ep08" --pre-check
```

For segment-level pre-check (scene mode, validate a segment draft before review):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\shanhong-zhixia-scene\episodes\ep08" --pre-check --content-file "G:\Auto-Storyboard\agent_runs\shanhong-zhixia-scene\episodes\ep08/segments/segXX/draft.txt"
```

Full validation command (run after review.txt and status.json are written):

```powershell
python "G:\Auto-Storyboard\storyboard_agent_workspace.py" validate-episode --episode-dir "G:\Auto-Storyboard\agent_runs\shanhong-zhixia-scene\episodes\ep08"
```
...
- `output_name`: `山洪之下-ep08-agent-cli-storyboard.txt`
