...
不再编译 `一句话概述`、`视觉峰值/特效重点`、`运镜强化词` 或 `Seedance执行提示补充`，也不使用“资产1/资产2”的扁平清单。动态特效不得因静态资产参考而缩水；特效、运镜、光影、声音和执行约束必须直接写入实际连续时间轴，时间轴仍应保留来源→形态→路径→作用对象→反馈→收束→声音的完整语义链。`seedance_submission_prompts.md` 是同一 `submission_prompt` 的人读镜像，方便生产人员在提交前逐组核对实际引用了哪些资产；它不是第二状态真源，也不能绕过 `submit_allowed`。
