...
class Seedance25LiveVerticalProfileTests(unittest.TestCase):
    ...
    def test_prepare_writes_profile_specific_skills_metadata_and_task_contract(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(episode["vertical_review_contract_version"], 4)
            ...
            self.assertIn("Vertical review contract: `v4`", task)
            self.assertIn("`review_facts.json` (generated by pre-check; mechanical content binding only; never hand-write)", task)
...
"""
            ...
            review = {
                ...
                "audit_coverage": {
                    key: "checked"
                    for key in saw._required_audit_coverage_keys(reviewer, 4)
                },
                "spot_checks": [
                ...
                "group_reviews": [
                    {
                        "group": "第1组",
                        "result": "pass",
                        "evidence": "甲延续上一集在客厅左侧持钥匙的状态，本组没有新增剧情或空间跳变。",
                    }
                ],
            ...
            self.assertEqual(review_facts["mechanical_evidence"]["group_count"], 1)
            self.assertNotIn("handoff_count", review_facts["mechanical_evidence"])
            self.assertNotIn("semantic_coverage", review)
            self.assertEqual(review["group_reviews"][0]["group"], "第1组")
