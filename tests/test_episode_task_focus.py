...
class EpisodeTaskFocusTests(unittest.TestCase):
    ...
    def test_seedance25_task_uses_agent_authored_v4_review_and_long_group_priority(self):
        task = saw.make_episode_task(
            run_dir=Path("H:/run"),
            episode_dir=Path("H:/run/episodes/ep01"),
            episode=saw.EpisodeInput(
        ...
        self.assertIn("one model-authored `group_reviews` entry per group", task)
        self.assertIn("v4 must not use `semantic_coverage`", task)
        ...
        self.assertIn("whole-episode review binding/coverage belong to step 4", task)
